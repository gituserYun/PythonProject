...
def dirSearch(target_url):
    ...
    #base_url: 기본 url(스키마 + 네트워크 IP)
    if target_url.startswith(base_url):
        #사용자의 동작을 통해 다른 웹 페이지나 웹 사이트로 이동시키는 태그
        for tag in soup.find_all(['script', 'a', 'form', 'link', 'img', 'area', 'meta', 'embed', 'object']):
            
            #script 태그 관련
            if tag.name == 'script':
                #location.href, window.location.href, location.replace(), window.location.replace(), ... 값 찾기
                match_location = re.search(r'location\s*\.\s*href\s*=\s*[\'"](.*?)[\'"]', tag.string, re.IGNORECASE)
                if tag.string:
                ...
                #ionicons와 같은 아이콘 리소스 참조 데이터는 분석 대상 제외
